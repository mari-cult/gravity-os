...
impl VfKeys {
    fn decrypt_chunk(&self, chunk_no: u32, data: &mut [u8]) -> std::io::Result<()> {
        ...
        let mut hmac = <Hmac<Sha1> as Mac>::new_from_slice(&self.hmac_key)
            .map_err(|e| std::io::Error::other(format!("HMAC init error: {}", e)))?;
        hmac.update(&chunk_no.to_be_bytes());
        let result = hmac.finalize();
        let digest = result.into_bytes();
        let iv = &digest[..16];

        let cipher = Aes128CbcDec::new_from_slices(&self.aes_key, iv)
            .map_err(|e| std::io::Error::other(format!("AES init error: {}", e)))?;

        cipher
            .decrypt_padded_mut::<block_padding::NoPadding>(&mut data[..decrypt_len])
            .map_err(|e| std::io::Error::other(format!("Decryption error: {}", e)))?;
        Ok(())
    }
}
...
impl<R: Read + Seek> VfDecryptor<R> {
    pub fn new(mut input: R, key_or_passphrase: &str) -> Result<Self> {
        ...
        if key_or_passphrase.len() >= 32 {
            ...
            if key_or_passphrase.len() >= 72 {
                for (i, hmac_key) in hmac_key_vec.iter_mut().enumerate() {
                    let start = 32 + i * 2;
                    *hmac_key = u8::from_str_radix(&key_or_passphrase[start..start + 2], 16)
                        .map_err(|e| VfDecryptError::HexDecode(e.to_string()))?;
                }
            }
    ...
    pub fn decrypt_parallel<W: Write>(&mut self, output: &mut W) -> std::io::Result<()> {
        ...
        loop {
            ...
            let num_chunks = n.div_ceil(self.block_size);
